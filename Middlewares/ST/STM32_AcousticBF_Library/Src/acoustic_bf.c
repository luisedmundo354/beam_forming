...
{
  return libBeamforming_getControl(pHandler, pControl);
}
...
/**
* @brief  Library setup function, it specifies the HW IP used for capturing PDM samples from microphones
* @param  pHandler: libBeamforming_Handler filled with desired parameters.
* @param  hwIP: selected HW IP.
* @retval 0 if everything is fine.
*/
uint32_t AcousticBF_SetHWIP(AcousticBF_Handler_t *pHandler, uint32_t hwIP)
{
  return libBeamforming_setHWIP(pHandler, hwIP);
}
...
{
  return AcousticBF_cardoid_GetLibVersion(version);
}
